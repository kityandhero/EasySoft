...
{
    private const string UniqueIdentifierAddAdvanceCap = "5cb2ec36-fa4f-4002-a28d-f80d41e69f85";
    ...
    {
        if (builder.HasRegistered(UniqueIdentifierAddAdvanceCap))
            return builder;
        ...
        builder.AddAdvanceCapCore();

        builder.Services.AddSingleton<IEventPublisher, CapPublisher>();
        builder.Services.AddScoped<MessageTrackerFactory>();
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    private static WebApplicationBuilder AddAdvanceCapCore(this WebApplicationBuilder builder)
    {
        ...
        StartupDescriptionMessageAssist.AddPrompt(
            $"Cap current switch is {(GeneralConfigAssist.CheckCapSwitch() ? "enable" : "disable")}",
            GeneralConfigAssist.GetConfigFileInfo()
        );
                    ...
                    switch (transportType)
                    {
                        ...
                        case TransportType.RabbitMQ:
                            ...
                            if (capConfig.Transport.RabbitMQ == null &&
                                (string.IsNullOrWhiteSpace(configFileHostName) ||
        ...
        if (GeneralConfigAssist.GetSkyApmSwitch())
        {
            StartupConfigMessageAssist.AddConfig(
                "SkyApmSwitch: enable.",
                GeneralConfigAssist.GetConfigFileInfo()
            ...
            builder.Services.AddSkyApmExtensions().AddCap();
        }
        else
        {
            StartupConfigMessageAssist.AddConfig(
                "SkyApmSwitch: disable.",
                GeneralConfigAssist.GetConfigFileInfo()
            );
        }
