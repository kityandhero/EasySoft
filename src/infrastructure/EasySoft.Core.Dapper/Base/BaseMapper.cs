...
using EasySoft.Core.Dapper.Entities;
...
namespace EasySoft.Core.Dapper.Base
{
    public class BaseMapper<T> : IMapper where T : IEntitySelf<T>, new()
    {
        ...
        public T? GetBy(ICollection<Condition<T>> conditions, IList<Sort<T>>? sorts, bool transferWrapperQuery = true)
        {
            ...
            {
                using IDbConnection conn = _mapperChannel.OpenConnection();
                try
                {
        ...
        private string AddCore(T model)
        {
            ...
            switch (typeCode)
            {
                ...
                case TypeCode.Int16:
                    if (Convert.ToInt16(primaryKeyValue) == 0)
                    {
                ...
                case TypeCode.Int32:
                    if (Convert.ToInt32(primaryKeyValue) == 0)
                    {
                        throw new Exception("主键没有赋值");
                    }
                ...
                case TypeCode.UInt16:
                    if (Convert.ToUInt16(primaryKeyValue) == 0)
                    {
                        throw new Exception("主键没有赋值");
                    }
                ...
                case TypeCode.UInt32:
                    if (Convert.ToUInt32(primaryKeyValue) == 0)
                    {
                        throw new Exception("主键没有赋值");
                    }
                ...
                case TypeCode.UInt64:
                    if (Convert.ToUInt64(primaryKeyValue) == 0)
                    {
                        throw new Exception("主键没有赋值");
                    }
                ...
                case TypeCode.Decimal:
                    if (Convert.ToDecimal(primaryKeyValue) == 0)
                    {
                        throw new Exception("主键没有赋值");
                    }
