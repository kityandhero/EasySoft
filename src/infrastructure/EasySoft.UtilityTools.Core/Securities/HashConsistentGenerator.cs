...
{
    private static HashConsistentGenerator _instance = new();

    public static HashConsistentGenerator Instance => _instance;
    ...
    public bool AddNode(params string[] hosts)
    {
        ...
        foreach (var item in hosts)
                 ...
                 i++) //此处循环为类似“192.168.3.1”这样的真实ip字符串从1加到1000，算作虚拟节点。192.168.3.11，192.168.3.11000
            {
                ...
                if (!VirtualNodeAndNodeMap
                        .ContainsKey(currentHash)) //因为hash可能会重复，如果当前hash已经包含在虚拟节点和真实节点映射中，则以第一次添加的为准，此处不再进行添加
                {
    ...
    public bool RemoveNode(string host)
    {
        ...
        for (var i = 1; i <= _virtualNodeMultiple; i++)
        {
            var currentHash =
                GetHashCode(host + i) &
                int.MaxValue; //计算一个hash，此处用自定义hash算法原因是字符串默认的哈希实现不保证对同一字符串获取hash时得到相同的值。和int.MaxValue进行位与操作是为了将获取到的hash值设置为正数
            if (VirtualNodeAndNodeMap.ContainsKey(currentHash) &&
                VirtualNodeAndNodeMap[currentHash] ==
                host) //因为hash可能会重复，所以此处判断在判断了哈希值是否存在于虚拟节点和节点映射中之后还需要判断通过当前hash值获取到的节点是否和指定节点一致，如果不一致，则证明这个这个虚拟节点不是当前hash值所拥有的
            {
