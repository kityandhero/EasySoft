...
public abstract class BaseCacheOperator : IAsyncCacheOperator
{
    ...
    {
        if (nameArray == null) throw new Exception("参数不能为null");

        if (nameArray.Length == 0) throw new Exception("不能构建空的缓存键名");
        ...
        if (string.IsNullOrWhiteSpace(result)) throw new Exception("构建缓存键名发生错误");
    ...
    public ExecutiveResult<T> Get<T>(string key)
    {
        ...
        if (!result.Success) return new ExecutiveResult<T>(ReturnCode.NoData);

        if (result.Data == null) return new ExecutiveResult<T>(ReturnCode.NoData);
        ...
        {
            if (result.Data is not SlidingWrapper<T> slidingWrapper) return new ExecutiveResult<T>(ReturnCode.NoData);

            var innerValue = slidingWrapper.Value;

            if (innerValue == null) return new ExecutiveResult<T>(ReturnCode.NoData);
    ...
    {
        if (initialTime <= TimeSpan.Zero) throw new Exception("初始过期时间值无效");

        if (slidingTime <= TimeSpan.Zero) throw new Exception("初始化滑动时间值无效");
    ...
    public void Set<T>(string key, T value, DateTime dateTime)
    {
        ...
        if (timeSpan.Milliseconds < 0) throw new Exception("设定的过期时间早于当前时间");
    ...
    {
        if (dateTimeMin < DateTime.Now) throw new Exception("设定的最小区间值不能早于当前");

        if (dateTimeMax < DateTime.Now) throw new Exception("设定的最大区间值不能早于当前");

        if (dateTimeMin < dateTimeMax) throw new Exception("设定的随机时间间隔区间无效");
    ...
    {
        if (expirationMin < expirationMax) throw new Exception("设定的随机时间间隔区间无效");

        if (expirationRatio <= 0) throw new Exception("设定的区间倍率无效,不能效于1");
    ...
    {
        if (moment < DateTime.Now) throw new Exception("指定的时刻不能早于当前");
