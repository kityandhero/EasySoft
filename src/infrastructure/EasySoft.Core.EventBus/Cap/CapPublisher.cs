...
public sealed class CapPublisher : IEventPublisher
{
    ...
    public async Task PublishAsync<T>(T eventObj, string? callbackName = null,
    ...
    public async Task PublishAsync<T>(T eventObj, IDictionary<string, string?> headers,
        CancellationToken cancellationToken = default)
        where T : class
    {
        await _eventBus.PublishAsync(typeof(T).Name, eventObj, headers, cancellationToken);
    }
