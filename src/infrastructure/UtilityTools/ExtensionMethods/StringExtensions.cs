        ...
        )
        {
            ...
            if (direction == StringCutDirection.FromRight)
            {
                return length >= fullLength
                    ? source
                    : string.Concat(showEllipsis, source.AsSpan(fullLength - length, length));
            }

            return length >= fullLength ? source : string.Concat(source.AsSpan(0, length), showEllipsis);
        }
