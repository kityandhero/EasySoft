...
public static class DateTimeExtensions
{
    ...
    public static DateTime BeginningOf(this DateTime date, TimeFrame timeFrame, CultureInfo? culture = null)
    {
        ...
        if (timeFrame == TimeFrame.Month) return new DateTime(date.Year, date.Month, 1);
    ...
    )
    {
        ...
        if (timeFrame != TimeFrame.Quarter) return date.BeginningOf(timeFrame, cultureAdjust);
    ...
    {
        if (calculateFrom == default) calculateFrom = DateTime.Now;
    ...
    public static int DaysIn(this DateTime date, TimeFrame timeFrame, CultureInfo? culture = null)
    {
        ...
        if (cultureAdjust == null) throw new Exception("culture is null");

        if (timeFrame == TimeFrame.Day) return 1;

        if (timeFrame == TimeFrame.Week) return 7;

        if (timeFrame == TimeFrame.Month) return cultureAdjust.Calendar.GetDaysInMonth(date.Year, date.Month);
    ...
    )
    {
        ...
        if (timeFrame != TimeFrame.Quarter) date.DaysIn(timeFrame, cultureAdjust);
    ...
    public static int DaysLeftIn(this DateTime date, TimeFrame timeFrame, CultureInfo? culture = null)
    {
        ...
        if (timeFrame == TimeFrame.Week) return 7 - ((int)date.DayOfWeek + 1);

        if (timeFrame == TimeFrame.Month) return date.DaysIn(TimeFrame.Month, cultureAdjust) - date.Day;
    ...
    )
    {
        ...
        if (timeFrame != TimeFrame.Quarter) return date.DaysLeftIn(timeFrame, cultureAdjust);
    ...
    public static DateTime EndOf(this DateTime date, TimeFrame timeFrame, CultureInfo? culture = null)
    {
        ...
        if (timeFrame == TimeFrame.Day) return new DateTime(date.Year, date.Month, date.Day, 23, 59, 59);

        if (timeFrame == TimeFrame.Week) return date.BeginningOf(TimeFrame.Week, culture).AddDays(6);
    ...
    )
    {
        ...
        if (timeFrame != TimeFrame.Quarter) return date.EndOf(timeFrame, cultureAdjust);
    ...
    {
        if (comparison.HasFlag(DateCompare.InFuture) && DateTime.Now >= date) return false;

        if (comparison.HasFlag(DateCompare.InPast) && DateTime.Now <= date) return false;

        if (comparison.HasFlag(DateCompare.Today) && DateTime.Today != date.Date) return false;
    ...
    {
        if (timeZone.IsNull()) throw new ArgumentNullException(nameof(timeZone));
